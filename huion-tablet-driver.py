...
class main():
    """
    """
    settings = {'pen_device_name':XINPUT_DEVICE_KEYWORD # must be defined here
...
def setup_driver():
    ...
    if main.settings['enable_calibrate_mapping']:
        print("\tCalibrate Mapping         ENABLED")
    else:
        print("\tCalibrate Mapping        disabled")
...
def read_config():
    ...
    try:
        main.settings['enable_calibrate_mapping'] = config.get('config', 'enable_calibrate_mapping')
    except:
        main.settings['enable_calibrate_mapping'] = False
